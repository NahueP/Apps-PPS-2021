...
export class CosasFeasPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.FirestoreService.ObtenerTodos('fotosFeas').subscribe(imagenesSnapShot=>{
  ...
  captureAndUploadToFirebaseStorage()
  {
    ...
    this.camera.getPicture(options).then((imagenData)=>{
      ...
      var nombreFoto = "cosasFeas/" + obtenerMs + "." + usuarioJson.nombre + ".jpg";
  ...
  {
    let auxLista = [];
    this.st.storage.ref('cosasFeas/').listAll().then((lista)=>{
  ...
  evaluarFoto()
  {
    ...
    this.listado.forEach(imagen =>{
      ...
      if(imagen.imagen == this.linkFoto)
      {
        ...
        if(!banderaVotos)
        {
          ...
          this.FirestoreService.Actualizar('fotosFeas', imagen, imagen.id);
    ...
    {
      this.FirestoreService.Crear('fotosFeas', foto);
      this.complementos.presentToastConMensaje("¡Votación Relizada!");
    }
  }
